...
from ..layers.utils import box_cxcywh_to_xyxy, box_iou, generalized_box_iou

from src.misc.dist import get_world_size, is_dist_available_and_initialized
from src.core import register
from src.losses.dice_loss import dice_loss


@register
class SparseSetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        losses = {
            "loss_mask": torchvision.ops.sigmoid_focal_loss(src_masks, target_masks, num_boxes),
    ...
    def get_cdn_matched_indices(dn_meta, targets):
        ...
        每张图片里面目标匹配到的对比学习样本的索引
        '''
        dn_positive_idx, dn_num_group = dn_meta["dn_positive_idx"], dn_meta["dn_num_group"]
        num_gts = [len(t['labels']) for t in targets] # 每张图片里面的target数量
...
@register
class OriginSetCriterion(nn.Module):
    """ This class computes the loss for DETR.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model
        2) we supervise each pair of matched ground-truth / prediction (supervise class and box)
