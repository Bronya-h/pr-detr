...
from token_prune.ddp import DiffRate
from token_prune.merge import get_merge_func

from token_prune.utils import ste_min
...
class DiffRateAttention(Attention):
    ...
    def forward(self, x: torch.Tensor, size: torch.Tensor = None, mask: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
...
def make_diffrate_class(transformer_class):
    ...
    class DiffRateVisionTransformer(transformer_class):
        ...
        def forward(self, x, return_flop=True) -> torch.Tensor:
            ...
            if self._diffrate_info["trace_source"]:
                self._diffrate_info["source"] = torch.eye(self.patch_embed.num_patches + 1, device=x.device)[None, ...].expand(B, self.patch_embed.num_patches + 1,
...
def apply_patch(model: VisionTransformer, trace_source: bool = False, prune_granularity=1, merge_granularity=1):
    """
