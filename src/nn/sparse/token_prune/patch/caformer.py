...
from token_prune.patch.deit import DiffRateAttention

from token_prune.utils import ste_min
from token_prune.merge import tokentofeature, uncompress
        ...
        if isinstance(self.token_mixer, DiffRateAttention):
            ...
            if self.training:
                ...
                if merge_kept_num < last_token_number:
                    ...
                    x_compressed, size_compressed, source_compressed = x[:,
                                                                         last_token_number:], self._diffrate_info["size"][:,
                                                                                                                          last_token_number:], self._diffrate_info["source"][:,
                                                                                                                                                                             last_token_number:]
...
def apply_patch(model: MetaFormer, prune_granularity=1, merge_granularity=1):
    """
