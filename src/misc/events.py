    ...
    def write(self):
        ...
        self.logger.info(
            str.format(
                ...
                iter=iteration,
                losses="  ".join(["{}: {:.4g}".format(k, v.median(storage.count_samples(k, self._window_size))) for k, v in storage.histories().items() if "loss" in k]),
                non_losses="  ".join(["{}: {:.4g}".format(k, v.median(storage.count_samples(k, self._window_size))) for k, v in storage.histories().items() if "[metric]" in k]),
                ...
                last_data_time="last_data_time: {:.4f}  ".format(last_data_time) if last_data_time is not None else "",
                lr=lr,
